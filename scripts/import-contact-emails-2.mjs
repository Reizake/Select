#!/usr/bin/env node
/**
 * Fuzzy-matches full_name from a CSV (columns: full_name, personal_email,
 * bethel_email, jwpub_email) against the candidates table and outputs:
 *   - matched2.sql    — UPDATE statements ready to run in Supabase SQL Editor
 *   - review2.csv     — nickname/fuzzy matches to spot-check
 *   - unmatched2.csv  — rows with no match; handle manually
 *
 * Usage:
 *   node scripts/import-contact-emails-2.mjs
 *
 * Requires NEXT_PUBLIC_SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY in environment
 * (or a .env.local file in the project root).
 *
 * Input CSV expected at: C:/dev/data/contactemail2.csv
...
const INPUT_CSV        = resolve(DATA_DIR, 'contactemail2.csv');
const OUTPUT_SQL       = resolve(DATA_DIR, 'matched2.sql');
const OUTPUT_REVIEW    = resolve(DATA_DIR, 'review2.csv');
const OUTPUT_UNMATCHED = resolve(DATA_DIR, 'unmatched2.csv');
...
function getVariants(name) { return nameToGroup.get(name) ?? new Set([name]); }
...
async function main() {
  ...
  for (const row of rows) {
    // CSV full_name is already "Last, First [M.]" — same format as DB
    const parsed = parseDBName(row.full_name);
    if (!parsed) {
      unmatched.push({ csv_name: row.full_name, personal_email: row.personal_email ?? '', bethel_email: row.bethel_email ?? '', jwpub_email: row.jwpub_email ?? '' });
      continue;
    }

    const result = findMatch(parsed.normFirst, parsed.normLast, index);

    const out = {
      csv_name:       row.full_name,
      personal_email: row.personal_email ?? '',
      bethel_email:   row.bethel_email ?? '',
      jwpub_email:    row.jwpub_email ?? '',
    ...
    if (!result) {
      ...
      matched.push(out);
      if (result.confidence !== 'exact') review.push(out);
    }
  }
  ...
  console.log(`  Nickname match: ${nickname}  → included in SQL + review2.csv`);
  console.log(`  Fuzzy match:    ${fuzzy}   → included in SQL + review2.csv`);
  console.log(`  Unmatched:      ${unmatched.length}  → unmatched2.csv`);

  // ── Write matched2.sql ──
  const sqlLines = [
    '-- Auto-generated by import-contact-emails-2.mjs',
    '-- All matches (exact + nickname + fuzzy) are included.',
    '-- review2.csv contains nickname/fuzzy rows — spot-check before running.',
    '',
    '-- ── PREVIEW ──────────────────────────────────────────────────────────────',
    `SELECT id, full_name, personal_email, bethel_email, jwpub_email FROM candidates WHERE id IN (`,
    ...
    'SET',
    '  personal_email = COALESCE(v.personal_email, c.personal_email),',
    '  bethel_email   = COALESCE(v.bethel_email,   c.bethel_email),',
    '  jwpub_email    = COALESCE(v.jwpub_email,     c.jwpub_email)',
    'FROM (VALUES',
    matched.map(r =>
      `  (${sqlStr(r.candidate_id)}, ${sqlStr(r.personal_email || null)}, ${sqlStr(r.bethel_email || null)}, ${sqlStr(r.jwpub_email || null)})`
    ).join(',\n'),
    `) AS v(id, personal_email, bethel_email, jwpub_email)`,
  ...
  // ── Write review2.csv ──
  const REVIEW_COLS = ['confidence', 'csv_name', 'db_name', 'personal_email', 'bethel_email', 'jwpub_email', 'candidate_id'];
  writeCSV(OUTPUT_REVIEW, review, REVIEW_COLS);

  // ── Write unmatched2.csv ──
  const UNMATCHED_COLS = ['csv_name', 'personal_email', 'bethel_email', 'jwpub_email'];
  writeCSV(OUTPUT_UNMATCHED, unmatched, UNMATCHED_COLS);

  console.log('\nDone.');
  console.log('  1. Skim review2.csv to catch any bad fuzzy matches');
  console.log('  2. Run the PREVIEW SELECT in Supabase SQL Editor');
  console.log('  3. Run the UPDATE block');
  console.log('  4. Check unmatched2.csv for anyone to handle manually');
}
